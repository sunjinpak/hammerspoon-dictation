...
- **Inline corrections feed back.** If you use the dictation inside a Claude Code prompt, `dictation_inline_learn.py` (a `UserPromptSubmit` hook) catches a dictated comment nested inside a dictation (`그<"그" 이건 빼도되.>`), logs the pair to `corpus/inline_learn.jsonl`, and the corrector receives the last ten as context. Your own words about what it got wrong become its instructions. Since v4.2 a nested comment that is itself a term (`레그드 릴레이션십<lagged relationship>`) is also written straight into `glossary/terms.tsv` as a permanent rule, taking as many preceding words as the term has, and the hook reports every capture over Telegram, saying whether it went to the glossary or only into the corrector context (it calls `~/.claude/channels/telegram/send.sh` if that script exists; otherwise it stays silent). Instruction-style comments ("drop this word") remain context-only. Test cases for this hook live in `v4/tests/test_inline_learn.py` (`python3 v4/tests/test_inline_learn.py`); each case is a real dictation that once went wrong, so add the next failure there before fixing it.
...
| `DICTATE_NOTIFY` | (none) | v4. Executable taking the nudge message as `$1`. Prints to stdout if unset. |
